...
Mat apply_filter_sequential(const Mat& src, const vector<vector<float>>& kernel) {
...
int main(int argc, char* argv[]) {
    ...
    // Test kernels
    vector<vector<float>> sharpen_kernel = {
        {0, -1, 0},
        {-1, 5, -1},
        {0, -1, 0}
    };
    
    vector<vector<float>> blur_kernel = {
    ...
    cout << "  Speedup: " << (double)seq_time.count() / par_time.count() << "x" << endl << endl;
