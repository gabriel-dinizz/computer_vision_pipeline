...
class OpenCVBaseline {
 ...
 public:
   ...
   */
  cv::Mat opencvGaussianBlur(const cv::Mat& img,
   ...
   */
  cv::Mat opencvBilateral(const cv::Mat& img, int d = 9, double sigmaColor = 75,
   ...
   * OpenCV Native CLAHE
   */
  cv::Mat opencvCLAHE(const cv::Mat& img, double clipLimit = 2.0,
...
int main(int argc, char** argv) {
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0]
              << " <input_img> <output_img> <filter_type>\n";
