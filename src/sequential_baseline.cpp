...
public:
     ...
     */
    cv::Mat sequentialGaussianBlur(const cv::Mat& img, cv::Size kernelSize = cv::Size(5,5), double sigma = 1.0) {
        cv::Mat result;
     ...
     */
    cv::Mat sequentialUnsharpMask(const cv::Mat& img, double sigma = 1.0, double strength = 1.5) {
     ...
     */
    cv::Mat sequentialBilateral(const cv::Mat& img, int d = 9, double sigmaColor = 75, double sigmaSpace = 75) {
        cv::Mat result;
     ...
     * Sequential CLAHE
     */
    cv::Mat sequentialCLAHE(const cv::Mat& img, double clipLimit = 2.0, cv::Size tileGridSize = cv::Size(8,8)) {
        cv::Mat result;
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        ...
        std::cerr << "Filter types: blur, sharpen, denoise, clahe, edge" << std::endl;
        return -1;
    }
    ...
    std::cout << "Sequential processing completed in " << duration.count() << " ms" << std::endl;
