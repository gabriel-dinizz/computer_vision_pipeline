...
private:
    ...
    static constexpr int PIXELS_PER_CACHE_LINE = CACHE_LINE_SIZE / sizeof(cv::Vec3b);
...
public:
    ...
    std::vector<float> generateGaussianKernel(int size, double sigma) const {
        ...
        perfCounters.kernelGenTime += std::chrono::duration<double, std::milli>(end - start).count();
    ...
    cv::Mat applyOptimizedGaussianBlur(const cv::Mat& img, double sigma = 1.0) {
        ...
        perfCounters.convolutionTime += std::chrono::duration<double, std::milli>(convEnd - convStart).count();
        ...
        perfCounters.totalTime += std::chrono::duration<double, std::milli>(totalEnd - totalStart).count();
        perfCounters.threadsUsed = omp_get_max_threads();
        
        if (verbose) std::cout << "Applied Optimized Separable Gaussian Blur (OpenMP)\n";
        return finalResult;
    }
     ...
     */
    cv::Mat applyOptimizedUnsharpMask(const cv::Mat& img, double sigma = 1.0, double strength = 1.5) {
        auto totalStart = std::chrono::high_resolution_clock::now();
        ...
        if (verbose) std::cout << "Applied Optimized Tile-based Unsharp Mask (OpenMP)\n";
        return finalResult;
    }
     ...
     */
    cv::Mat applyOptimizedBilateralFilter(const cv::Mat& img, int d = 9, double sigmaColor = 75, double sigmaSpace = 75) {
        auto totalStart = std::chrono::high_resolution_clock::now();
        ...
        for (int dy = -radius; dy <= radius; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
                ...
                spatialWeights[dy + radius][dx + radius] = std::exp(-spatialDist * spatialSigmaInv);
            }
        }
        
        // Color sigma for bilateral weight calculation
        float colorSigmaInv = 1.0f / (2.0f * sigmaColor * sigmaColor / (255.0f * 255.0f));
            ...
            for (int x = 0; x < img.cols; x++) {
                ...
                if (totalWeight > 0) {
                    ...
                    resultRow[x] = cv::Vec3b(
                        cv::saturate_cast<uchar>(filtered[0] * 255.0f),
        ...
        if (verbose) std::cout << "Applied Optimized Memory-Efficient Bilateral Filter (OpenMP)\n";
        return result;
    }
     ...
     */
    cv::Mat applyOptimizedCLAHE(const cv::Mat& img, double clipLimit = 2.0, cv::Size tileGridSize = cv::Size(8,8)) {
        auto totalStart = std::chrono::high_resolution_clock::now();
    ...
    void printPerformanceAnalysis() const {
        if (verbose) {
            ...
            std::cout << "Total processing time: " << std::fixed << std::setprecision(2) << perfCounters.totalTime << " ms\n";
            std::cout << "Convolution time: " << perfCounters.convolutionTime << " ms (" 
                      << (perfCounters.convolutionTime / perfCounters.totalTime * 100) << "%)\n";
            std::cout << "Kernel generation time: " << perfCounters.kernelGenTime << " ms\n";
            std::cout << "Memory allocation time: " << perfCounters.memoryAllocTime << " ms\n";
            std::cout << "Threads utilized: " << perfCounters.threadsUsed << "\n";
            std::cout << "Processing efficiency: " << (100.0 / perfCounters.threadsUsed) << "% per thread\n";
            std::cout << "============================\n";
        }
    }
