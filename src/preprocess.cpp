...
#pragma omp parallel for
   ...
   */
  cv::Mat applyUnsharpMask(const cv::Mat& img, double sigma = 1.0,
                           double strength = 1.5) {
...
#pragma omp parallel for collapse(2)
   ...
   */
  cv::Mat applyBilateralDenoise(const cv::Mat& img, int d = 9,
...
#pragma omp parallel for
    for (int i = 0; i < 3; ++i) {
      cv::bilateralFilter(channels[i], filteredChannels[i], d, sigmaColor,
                          sigmaSpace);
    }
   ...
   */
  cv::Mat applyCLAHE(const cv::Mat& img, double clipLimit = 2.0,
