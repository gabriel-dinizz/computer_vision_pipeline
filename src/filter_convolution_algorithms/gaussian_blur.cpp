...
int main() {
  try {
    ...
    if (image.empty()) {
      std::cout << "Could not load image. Testing with synthetic data..."
                << std::endl;
      ...
      cv::Mat separableBlurred =
          gaussianFilter.applySeparableConvolution(testImage);
      ...
      cv::Mat separableBlurred =
          gaussianFilter.applySeparableConvolution(image);
