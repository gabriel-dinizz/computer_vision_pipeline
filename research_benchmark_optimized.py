...
class OptimizedOpenMPBenchmark:
    ...
    def _warmup_system(self):
        ...
        if os.path.exists(warmup_image):
            for _ in range(3):
                try:
                    cmd = [
                        "./bin/preprocess_optimized",
                        warmup_image,
                        "/tmp/warmup_output.jpg",
                        "blur",
                    ]
    ...
    def calculate_comprehensive_metrics(
        self, sequential_result, original_results, optimized_results, opencv_result
    ):
        ...
        for threads in self.thread_counts:
            ...
            orig_result = next(
                (r for r in original_results if r["threads"] == threads), None
            )
            opt_result = next(
                (r for r in optimized_results if r["threads"] == threads), None
            )
            ...
            orig_vs_seq_tstat, orig_vs_seq_pval = stats.ttest_ind(
                sequential_result["times"], orig_result["times"]
            )
            opt_vs_seq_tstat, opt_vs_seq_pval = stats.ttest_ind(
                sequential_result["times"], opt_result["times"]
            )
            opt_vs_orig_tstat, opt_vs_orig_pval = stats.ttest_ind(
                orig_result["times"], opt_result["times"]
            )
            ...
            analysis["thread_analysis"].append(
                {
                    ...
                    "improvement_analysis": {
                        ...
                        "optimized_vs_original_time_reduction": (
    ...
    def generate_comparison_plots(self, analysis):
        ...
        ax1.plot(
            threads, orig_times, "o-", linewidth=2, label="Original OpenMP", color="red"
        )
        ax1.plot(
            threads,
            opt_times,
            "s-",
            linewidth=2,
            label="Optimized OpenMP",
            color="green",
        )
        ax1.axhline(
            y=analysis["baseline_time_ms"],
            color="blue",
            linestyle="--",
            label="Sequential Baseline",
        )
        ax1.axhline(
            y=analysis["opencv_time_ms"],
            color="orange",
            linestyle="--",
            label="OpenCV Native",
        )
        ...
        ax2.plot(
            threads, threads, "--", alpha=0.7, label="Ideal Speedup", color="black"
        )
        ...
        orig_efficiencies = [
            d["original_openmp"]["efficiency_percentage"] for d in thread_data
        ]
        opt_efficiencies = [
            d["optimized_openmp"]["efficiency_percentage"] for d in thread_data
        ]
        ...
        ax3.axhline(
            y=100, color="black", linestyle="--", alpha=0.7, label="Ideal Efficiency"
        )
        ...
        improvements = [
            d["improvement_analysis"]["optimized_vs_original_speedup"]
        ...
        ax4.axhline(
            y=1, color="black", linestyle="--", alpha=0.7, label="No Improvement"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.results_dir / f"{filter_name}_optimization_comparison.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def generate_research_report(self, all_analyses):
        ...
        report = {
            "study_metadata": {
                ...
                "total_measurements": len(self.filters)
        ...
        for analysis in all_analyses:
            ...
            best_orig = min(
                thread_data, key=lambda x: x["original_openmp"]["mean_time_ms"]
            )
            best_opt = min(
                thread_data, key=lambda x: x["optimized_openmp"]["mean_time_ms"]
            )
            ...
            report["optimization_summary"][filter_name] = {
                ...
                "optimization_impact": {
                    ...
                    "optimization_successful": max_improvement
                    > 1.1,  # 10% improvement threshold
                },
                "opencv_comparison": {
                    "original_vs_opencv": analysis["baseline_time_ms"]
                    / analysis["opencv_time_ms"],
                    "optimized_vs_opencv_best": best_opt["vs_opencv_comparison"][
                        "optimized_vs_opencv"
                    ],
                },
            }
    ...
    def run_complete_optimization_study(self, image_path):
        ...
        print(
            "🔄 Comparing: Sequential, Original OpenMP, Optimized OpenMP, OpenCV Native"
        )
        print()
        ...
        for filter_type in self.filters:
            ...
            for threads in self.thread_counts:
                ...
                orig_result = self.run_original_parallel_benchmark(
                    image_path, filter_type, threads
                )
            ...
            for threads in self.thread_counts:
                ...
                opt_result = self.run_optimized_parallel_benchmark(
                    image_path, filter_type, threads
                )
            ...
            # 7. Save raw data
            all_results = (
                [sequential_result, opencv_result]
                + original_results
                + optimized_results
            )
            df = pd.DataFrame(all_results)
            df.to_csv(
                self.results_dir / f"{filter_type}_optimization_raw_data.csv",
                index=False,
            )
            ...
            if analysis["thread_analysis"]:
                ...
                best_opt_speedup = max(
                    [
                        d["optimized_openmp"]["speedup"]
                        for d in analysis["thread_analysis"]
                    ]
                )
                print(f"     Best optimization improvement: {best_improvement:.2f}x")
                print(
                    f"     Best optimized speedup vs sequential: {best_opt_speedup:.2f}x"
                )
            print()
        ...
        for filter_name, summary in report["optimization_summary"].items():
            ...
            print(
                f"    Original Best Speedup: {summary['best_original_performance']['speedup']:.2f}x"
            )
            print(
                f"    Optimized Best Speedup: {summary['best_optimized_performance']['speedup']:.2f}x"
            )
            print(
                f"    Max Improvement: {summary['optimization_impact']['max_improvement_factor']:.2f}x"
            )
            success = (
                "✅ SUCCESS"
                if summary["optimization_impact"]["optimization_successful"]
                else "❌ NEEDS WORK"
            )
...
if __name__ == "__main__":
    ...
    benchmark.run_complete_optimization_study(
        "images/2019_Toyota_Corolla_Icon_Tech_VVT-i_Hybrid_1.8.jpg"
    )
