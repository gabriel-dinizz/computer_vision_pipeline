...
class VisionPipeline:
    ...
    def preprocess_image(
        self, input_path, output_path=None, filter_type="auto", verbose=True
    ):
        """
        ...
        if not self.preprocess_bin.exists():
            ...
            return self._python_fallback_preprocess(
                input_path, output_path, filter_type
            )
    ...
    def run_yolo_detection(
        self, image_path, output_dir=None, confidence=0.25, device="cpu"
    ):
        """
    ...
    def run_full_pipeline(
        ...
        preprocessed_path = self.preprocess_image(
            input_path, filter_type=filter_type, verbose=verbose
        )
        ...
        return {
            ...
            "processing_time": {"preprocessing": preprocess_time, "total": total_time},
        }


def main():
    parser = argparse.ArgumentParser(
        description="Computer Vision Pipeline with Preprocessing and YOLO Detection"
    )
    ...
    parser.add_argument(
        "-c", "--confidence", type=float, default=0.25, help="YOLO confidence threshold"
    )
    parser.add_argument(
        "-d",
        "--device",
