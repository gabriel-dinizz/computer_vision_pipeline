...
class AcademicCVBenchmark:
    ...
    def generate_academic_report(self, results: Dict, image_path: str):
        ...
        plt.tight_layout()
        plt.savefig(self.results_dir / 'preprocessing_performance.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(self.results_dir / 'speedup_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _generate_summary_report(self, results: Dict, image_path: str):
        ...
        with open(report_file, 'w') as f:
            ...
            if speedups and efficiencies:
                ...
                f.write(f"- **Performance gain:** {((avg_speedup-1)*100):.1f}% improvement with 8 threads\n\n")
            ...
            f.write(f"{self.iterations_per_test} times with {self.warmup_iterations} warmup iterations. ")
            f.write("Thread affinity and controlled environment ensure reproducible results.\n\n")
    ...
    def run_complete_study(self, image_path: str) -> Dict:
        ...
        if not self.preprocess_bin.exists():
            raise FileNotFoundError(f"Preprocessing binary not found: {self.preprocess_bin}")
        ...
        print(f"\n🏆 Complete study finished! Results in {self.results_dir}/")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Academic CV Pipeline Benchmark")
    parser.add_argument("--image", default="../images/sample.jpg", help="Test image path")
    parser.add_argument("--preprocess-bin", default="../bin/preprocess_optimized", help="Preprocessing binary")
    ...
    try:
        results = benchmark.run_complete_study(args.image)
